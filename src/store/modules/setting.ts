...
export const useSettingStore = defineStore(
  ...
  () => {
    ...
    const getMenuOpenWidth = computed((): string => {
      return menuOpenWidth.value ? `${menuOpenWidth.value}px` : `${SETTING_DEFAULT_CONFIG.menuOpenWidth}px`
    })
    ...
    const getCustomRadius = computed((): string => {
      return customRadius.value ? `${customRadius.value}rem` : `${SETTING_DEFAULT_CONFIG.customRadius}rem`
    })
