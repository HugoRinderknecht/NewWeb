    ...
    <div v-if="!isLock">
      <ElDialog v-model="visible" :width="370" :show-close="false" @open="handleDialogOpen">
        <div class="flex-c flex-col">
          ...
          <div class="mt-7.5 mb-3.5 text-base font-medium">{{ userInfo.username || userInfo.userName }}</div>
          <ElForm
            ref="formRef"
            :model="formData"
            :rules="rules"
...
<script setup lang="ts">
  ...
  const { info: userInfo, isLock } = storeToRefs(userStore)
  ...
  const handleUnlock = async () => {
    ...
    await unlockFormRef.value.validate(async (valid, fields) => {
      if (valid) {
        const isValid = await userStore.verifyLockPassword(unlockForm.password)
