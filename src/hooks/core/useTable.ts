...
function useTableImpl<TApiFn extends (params: any) => Promise<any>>(
  ...
  const refreshRemove = async (): Promise<void> => {
    ...
    // 如果当前页为空且不是第一页，回到上一页
    if (data.value.length === 0 && (current as number) > 1) {
      pagination.current = (current as number) - 1
      ;(searchParams as Record<string, unknown>)[pageKey] = (current as number) - 1
      await getData()
    }
  }
